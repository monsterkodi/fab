...
const GHOST_MATERIAL = preload("uid://dlc24r3ng1x47")
...
func updateCorners():
    ...
    corners.get_child(0).global_position = Vector3(maxx+0.5, 0.15, miny-0.5)
    corners.get_child(1).global_position = Vector3(maxx+0.5, 0.15, maxy+0.5)
    corners.get_child(2).global_position = Vector3(minx-0.5, 0.15, maxy+0.5)
    corners.get_child(3).global_position = Vector3(minx-0.5, 0.15, miny-0.5)
