...
func tilt_delta(delta:float):
    ...
    if delta < 0:
        ...
        delta = min(deg_to_rad(100)-angle, delta)
...
func readInput():
    ...
    tilt = -deadZoneAxis(JOY_AXIS_RIGHT_Y)
    if Input.is_action_pressed("tilt_up"):      tilt += 1
    if Input.is_action_pressed("tilt_down"):    tilt -= 1
...
func _input(e: InputEvent):
    ...
    if e is InputEventMouseMotion:
        ...
        if Input.is_mouse_button_pressed(MOUSE_BUTTON_RIGHT):
            ...
            steer_delta(e.relative.x * 0.002)
